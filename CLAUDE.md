...
**The job name is the first line of the file, bare, and the banner is gone**
(2026-08-21). The header opens `%` then `(T24 Test)` — no `Job:` label and no
`(MASTER JOB - Generated by CNC Nest Tool)`. Every file this app writes is a generated
master, so the banner distinguished nothing while pushing the one line that identifies
the program down to line 4, and the first thing an operator does with an opened program
is check they loaded the right one. The name is the same string as the file name, so it
either matches at a glance or it does not. **It goes after the `%`, not before it**:
the tape marks bracket the program and a comment outside them is one some controls drop
on load, which would lose exactly the line that has to survive — and with line 1 a lone
`%`, this is still the first line with anything to read.

**The header states the estimated cycle time — the total, and nothing else**
(2026-08-21). `(Est cycle time: 10m 53s)`. It carried the per-bucket breakdown and the
per-event unit costs for one afternoon and they were cut the same day (operator): at
the control, the total is what anyone reads. The detail is not lost, it moved to where
it means something — `<job>_setup.txt` states the touch-off posture as an instruction
*and* the same cycle time, which is the right home for "this assumed 27 s a change"
because it sits beside the setting it depends on, and `calibrate_runtime.py` prints
every bucket per file. Don't put them back in the header.

`with_runtime_header` is a separate call from `generate_master_gcode` for a reason the
chicken-and-egg makes plain: the estimate is derived from the finished file, so the
generator cannot know it while writing one, and `app.api_generate` therefore takes
**one** estimate and hands the same dict to the header, `<job>_setup.txt` and the PDF.
A second walk inside the generator would be a second number for one program. Inserting
a comment cannot change what the comment says (the estimator skips comments) and it
carries no `N` word, so nothing is renumbered — but the insert happens **before**
`validate_gcode`, so the findings' line numbers point into the file that actually gets
written.
...
`Y2727.7000.`, two decimal points in one word. Four of the six Fusion files in the library carry such a word inside a pass
...
**The mid-cut alarm this was first justified by never happened, and the rule stands
anyway** (2026-08-21). The claim was inferred rather than observed, and the observation
went the other way: `tests/fixtures/t24-test.nc` carries **five** doubled decimal points
and ran a full clean cycle on the SS2 — no alarm, no feed hold. So the Syntec's parser
is forgiving here. Do not soften the checks on that news. What a forgiving parser does
with a malformed word is its own business — take the value, take part of it, take a
different control's reading of it — and the app has no way to tell which; a generator
whose output only one control will accept is a generator nobody can check. The severity
rests on "a word whose grammar the app cannot defend is one it must not write", which is
an argument about the generator, not about this control.
...
tokeniser cannot consume is an **ERROR** and the file is never written. It stays quiet
on a block whose stripped form still holds a paren: that is a broken comment, and the
comment check names the real problem rather than reporting the same defect twice.
...
**`runtime_estimator.py`** — walks a line stream tracking modal units, motion mode,
plane, feedrate and XYZ; builds a segment list; plans it with look-ahead; and returns
...
(2026-08-20). `distance / feedrate` prices a machine that reaches full speed instantly
and turns corners without slowing. On the `T24 Test` nest the **rapids alone** go from
25 s to 2m52s once acceleration is modelled, on identical geometry, and the whole
window from 7m20s to a measured 10m53s. The shop's files are why: the median G1 XY
move in that file is **0.495 mm**, which at F2540 lasts 12 ms — not long enough to
reach F2540. The model is a trapezoidal velocity profile with GRBL-style
junction-deviation look-ahead, in three stages that share nothing but a segment list —
`_scan_segments` knows G-code and no physics, `_plan_velocities` and `_integrate` know
physics and no G-code, which is what lets the physics be tested without writing G-code.

**No constant factor can be substituted for it.** Measured across the library before
the model was fitted, the correction ranged from 1.11× to 2.11× per file. It falls out
of segment length and corner angle, so a single multiplier fits one job and is wrong on
the next.

**`accel = float("inf")` reproduces the old arithmetic exactly**, and that is
load-bearing rather than a curiosity: every test that asserts `distance / feedrate` to
1e-6 is about *geometry* — arc sweep, unit words, modal motion, bucket sums — so those
tests run under `_INSTANT` and keep pinning exactly what they were written to pin.
Recalibrating the acceleration therefore cannot break a geometry test. The physics gets
its own tests, written to hold for **any** acceleration (`t = L/v + v/a` for a long
move, `2·sqrt(L/a)` for a short one, a reversal costing two isolated moves).

**`DEFAULT_ACCEL_MM_S2` is an EFFECTIVE acceleration, not the machine's parameter.**
189 mm/s² is about 0.019 g, far too low for a 5×10 gantry, because it is absorbing
everything else that slows a dense toolpath and is not modelled: jerk limiting, the
control's block processing rate, and a look-ahead buffer of a few hundred blocks rather
than the whole file. **A disagreement with a figure read off the Syntec is expected, not
a bug — do not "correct" it against one.** The lumping is deliberate: priced instead as
a plausible 1000 mm/s² plus a per-block floor, the reference file needs 29 ms/block, and
across the library the two models disagree by at most 9%. A second parameter that one
observation cannot separate is worse than none.

**A cost the model does not name is a cost acceleration ends up wearing, and that is
what moved this number 240 → 189** (2026-08-21). Two corrections, both in the same
direction, found the same day:

- **−60 s, the posture.** The 240 fit charged `(9) 18G Test` 2 × 57 s on the strength
  of issue #8's *decision* that auto tool touch-off stays on — nobody wrote down what
  the control was actually set to. If it was off, the same 697 s buys 60 s more
  **motion**, which is 9% of the job, and it went into acceleration instead. **A timed
  job with no posture recorded beside it is worth roughly nothing.**
- **−20 s, spindle spin-up**, which nothing was charging at all (see
  `SPINDLE_START_SECONDS` below).

`T24 Test` is the fit now (653 s, 2026-08-21, touch-off **off**, cycle start → the
end-of-job change) because both of those are known for it, and the older job survives
as a cross-check: at 189 it predicts 674 s against 697 s, 3.4% low, where the on
reading is 5.2% high. Both tests hold their job at 27 s explicitly rather than reading
`config.json`, because **the posture is part of the observation** and an operator
flipping the Settings checkbox must not re-price a measurement after the fact.

It is still **one parameter fitted to one observation**, so the agreement is exact by
construction. Acceleration is the only sensitive knob — ±10% moves the total ∓1.5% —
while junction deviation is nearly inert (0.01 → 0.1 mm moves it 1%), which is what
makes a one-number fit meaningful at all. Re-fit with `calibrate_runtime.py`, which
takes `(file, observed)` pairs and **prints the number rather than writing it**: a fit
over two or three observations is a judgement, and a script that edits the machine model
silently is how a bad fit becomes invisible (the same reasoning that keeps `pocket_map`
from writing back a default slot). `--no-auto-tool` prices a job timed with the posture
off and `--to-end-of-job-change` matches the window an operator can actually watch — the
cut finishing, before the carousel and the park. `--check` predicts the library at the
current config and ranks the files by how much their prediction moves with accel; a job
whose prediction barely moves cannot tell you what accel is.

**What the two observations still cannot separate: acceleration from a fixed per-job
overhead.** Both jobs are ~11 minutes, so "motion is N% slower than modelled" and
"every job costs a constant the model never sees" still fit about equally well —
program load and the operator's start delay are real and unmodelled. Naming the
spindle spin-up took the largest identifiable piece out of that pool, and it is *not*
the same shape (per tool block, not per job), but the remainder is unresolved. **One
timed job of a different length settles it**: on a 3-minute job the first story
predicts a ~20 s miss and the second a ~55 s one. Time a short job next, not another
11-minute one, and record the posture with it. Until then this stays one free
parameter, and `calibrate_runtime.py` prints the constant as a diagnostic it
deliberately does not adopt.

**Three things the old walk got wrong were fixed in the same pass**, because each is
invisible until the model is otherwise right:

- **A Z-only `G1` plunge is timed, not skipped.** "Ignored by convention" was a
  convention with nothing behind it: a 19 mm plunge at F100 is 11 s, dozens per part,
  all free. `F` is the resultant feed along the **3D** path on a Fanuc/Syntec, so a
  ramp is timed on its true length too.
- **The modal plane is tracked** (`gcode_parser.plane_mode`), because `_arc_length` read
  only `I`/`J` and a G19 arc's centre offsets are `J`/`K` — every vertical lead-in and
  lead-out ramp timed at zero, **45 of the 162 arcs** in the reference file. Same lesson
  as modal motion, one level down: VCarve writes the plane word on a line of its own.
  `plane_mode` lives beside `motion_mode` and `extract_file_segments` reads it too;
  `gcode_generator` and `gcode_validator` keep their own copies **on purpose** — the
  generator's is in file-vs-machine terms because it *rewrites* the word, and the
  validator deliberately shares no state with what it checks.
- **A tool change stops the look-ahead dead** (a zero-length `"stop"` segment, so
  neither planning pass needs a special case). Otherwise the planner carries speed
...
rates. Don't add `z_rapid_ipm` back.

**Two known approximations, both left deliberately.** The walk starts each file at the
origin, so a file's first rapid is fictional (the machine is at the changer) and
acceleration made it more expensive — `extract_file_segments` makes the opposite choice
because a fictional line in a *preview* is a visible defect while a fictional rapid in a
*total* is a small one. And the look-ahead plans over the whole file where the control
buffers a few hundred blocks, so the model is optimistic on dense 3D paths; that error
is inside the fitted acceleration.
...
across parts, and neither does its own spindle-start count, so both are charged once
per emitted block at job level instead. `app._reprice_loaded_parts` zeroes the same
two — it is the other producer of that field, and a part repriced on different terms
than it was parsed on is a job total built from two machines. The `.txt` report reads
the finished file and is the precise figure; `_compute_job_stats` is the live
approximation.

**Spindle spin-up is 5 s, charged on every `S` word** (`SPINDLE_START_SECONDS`,
observed 2026-08-21). Every time the control sees an `S` it holds for about 5 s — not
only when the speed changes, and not only from a stop. It is a **measured** cost, like
the two tool-change constants, which is what lets it exist beside the one *fitted*
parameter: the rule against a second parameter is about free ones, and folding an
observation into the acceleration is precisely the mistake the posture confound made.
It is charged per `S`, which is per **tool block** — `gcode_generator._dedup_spindle`
leaves exactly one per block, so that function is now worth 5 s per line it drops
rather than being tidiness. The live panel charges `len(block_tools)` starts, not
`tool_changes`: the end-of-job change loads a tool and cuts nothing, so it spins
nothing up. Each start is also a look-ahead barrier, for the same reason a tool change
is — the machine is standing still while the spindle comes up.
...
tool_change_count`, every run); collapsing the two constants deletes the record of an
...
`accel_mm_s2`, `junction_deviation_mm` and `auto_tool_touch_off` are what
`runtime_estimator` runs on. `rapid_ipm` is in **inches** per minute because that is how
the machine quotes it and how it would be re-measured; `MotionLimits.from_config` is the
one place it becomes mm/min, exactly as `edge_margin_in` and `locating_pins[].x_in` keep
their measured units. `app._motion_limits()` and `app._tool_change_seconds()` are the
single readers, matching `_bed_x` / `_rails` / `_tool_capacity`, and every read supplies
its own fallback because `config.py` has no defaults dict — a config predating these
keys must behave as the shipped machine model, and a missing `auto_tool_touch_off` must
read as **on**.

**The limits are threaded, never read from a module global, and a config save re-prices
what is already loaded.** `parse_vcarve_text` takes a `limits` courier it never looks
inside, because `gcode_parser` has no config access and a per-part runtime computed
under a different machine model than the job total is one job quoting two machines. For
the same reason `api_config_post` calls `_reprice_loaded_parts`: `PlacedPart.part` is
the *same object* as `_loaded[rel]`, so without it an edited acceleration would leave
every cached `runtime_seconds` on the old model while the tool-change term used the new
one. Nothing is re-read from disk — only the physics changed.
...
4. User clicks Generate → `/api/generate` gates on the §3.4 validity rules →
   ...
   for the pocket map → the file is priced once and the estimate goes into its header →
   `gcode_validator` reads the result → writes `.nc`, `.pdf`, `<job>_setup.txt`
