...
description = "SYNTEC - Refine Modified";
vendor = "SYNTEC";
...
// --- REFINE CUSTOM: stock header ---
function fmt3(v) {
  return (Math.round(v * 1000) / 1000).toFixed(3);
}

function writeStockInfo() {
  writeln("(REFINE POST ACTIVE)");
  var wp = getWorkpiece();
  if (!wp) { writeln("(NO WORKPIECE RETURNED)"); return; }
  writeln("( Material Size)");
  writeln("( X= " + fmt3(wp.upper.x - wp.lower.x) +
          ", Y= " + fmt3(wp.upper.y - wp.lower.y) +
          ", Z= " + fmt3(wp.upper.z - wp.lower.z) + ")");
}
...
function onOpen() {
  ...
  writeBlock(gAbsIncModal.format(90), gFeedModeModal.format(getProperty("useG95") ? 95 : 94), gPlaneModal.format(17), lengthCompOutput.format(lengthCompCodes.cancel), gFormat.format(40), gFormat.format(80));
...
function writeToolIdentity(tool) {
  ...
  if (tool.description) {
    writeComment("TOOLDESC " + t + " " + tool.description);
  }
}
