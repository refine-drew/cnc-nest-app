...
# Four of the six Fusion files in the library carry such a word inside a pass
# body, so this is the normal case, not an edge one.
#
# **The Syntec turns out to read such a word without complaint** — the timed job
# in `tests/fixtures/t24-test.nc` carries five of them and ran clean on
# 2026-08-21 — so the mid-cut alarm this comment first claimed was inferred, not
# observed. That changes nothing here or in `gcode_validator._check_word_syntax`,
# which still rejects one at ERROR: a word whose grammar the app cannot defend is
# one it must not write, and "this control happens to be forgiving about it" is
# not a property to build a G-code generator on.
...
def generate_master_gcode(placements: List[PlacedPart], settings: Dict,
    ...
    filenames = [_part_label(p.part.filename) for p in placements]
    ...
    #
    # **The job name is the first thing in the file, bare and unlabelled**, because
    ...
    # matches at a glance or it does not. It sits *after* the `%` rather than before
    # it: the tape marks bracket the program, and a comment outside them is a comment
    # some controls drop on load, which would lose exactly the line that has to
    # survive. Line 1 being a lone `%`, that still leaves this the first line anyone
    # reads. A `Job:` label and a `(MASTER JOB - Generated by CNC Nest Tool)` banner
    # were both dropped on 2026-08-21 (operator): every file this app writes is a
    # generated master, so the banner distinguished nothing, and it pushed the one
    # line that identifies the program to line 4.
    out += [
        "%",
        comment(job_name),
        comment(f"Date: {now}"),
...
# The two extensions the library holds. Stripped from the `(Parts: ...)` line and
# nowhere else: every part in the list is a posted G-code file, so the suffix is the
# same word repeated once per part, and on a nest of nine it is most of the line.
# Matched explicitly rather than by splitting on the last dot, which would take the
# `.2` off a part named `Rail v1.2.nc`.
_PART_EXTENSION = re.compile(r"\.(?:nc|mmg)$", re.IGNORECASE)


def _part_label(filename: str) -> str:
    """A part's name as the header lists it — the file name without its extension."""
    return _PART_EXTENSION.sub("", filename)


# ── the cycle-time line in the header ────────────────────────────────────────

RUNTIME_COMMENT_PREFIX = "Est cycle time:"


def runtime_comment(runtime: Dict) -> str:
    """The header's cycle-time line: the total, and nothing else.

    It carried the buckets and the per-event unit costs for one afternoon
    (2026-08-21) and they were cut the same day — at the control, the total is what
    anyone reads. The detail is not lost, it is just not here: `<job>_setup.txt`
    states the touch-off posture as an instruction *and* the same cycle time, which
    is the right place for "this number assumed 27 s a change" because it sits next
    to the setting it is talking about. `calibrate_runtime.py` is where the buckets
    matter, and it prints them per file.

    "Est" is doing real work: `DEFAULT_ACCEL_MM_S2` is fitted to one timed job.
    """
    return comment(f"{RUNTIME_COMMENT_PREFIX} {format_duration(runtime['seconds'])}")


def with_runtime_header(gcode: str, runtime: Dict) -> str:
    """Add the cycle-time line to the end of an emitted file's header.

    **It goes in afterwards, and that is the point.** The estimate is derived from
    the finished file, so the generator cannot know it while it is still writing
    one; and doing it here means the *same* estimate reaches the header, the setup
    sheet and the PDF, rather than the header carrying a second walk of the same
    file that could disagree with the other two about one program.

    Inserting a comment cannot change the number it states — the estimator skips
    comment lines — and it carries no `N` word, so no block is renumbered.

    It lands after the last header comment, found by scanning rather than by
    counting: the header's other lines are the caller's to change.
    """
    lines = gcode.split("\n")
    at = 0
    while at < len(lines) and (lines[at].startswith("%") or lines[at].startswith("(")):
        at += 1
    lines.insert(at, runtime_comment(runtime))
    return "\n".join(lines)
